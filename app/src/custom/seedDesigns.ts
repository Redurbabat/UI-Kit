...
const focusButtonHtml = `<div class="btn-wrapper">
  <button class="btn" type="button">
    <span class="txt-wrapper">
      <span class="txt-1">${'FOCUS'.split('').map((letter) => `<span class="btn-letter">${letter}</span>`).join('')}</span>
      <span class="txt-2">READY</span>
    </span>
    <svg class="btn-svg" viewBox="0 0 24 24" aria-hidden="true">
      <path d="M5 12h12m-5-5 5 5-5 5" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" />
    </svg>
  </button>
</div>`

const focusButtonCss = String.raw`.btn-wrapper { 
  position: relative; 
...
const focusButtonPreviewCss = focusButtonCss.replace(
  'hsla(var(--highlight-color-hue), 100%, 70%, 50%), \n    8%, \n    transparent',
  'hsla(var(--highlight-color-hue), 100%, 70%, 50%) 8%, \n    transparent',
)

const dominoHtml = `<div class="spinner" aria-label="Loading">
  <span></span><span></span><span></span><span></span>
  <span></span><span></span><span></span><span></span>
</div>`

const dominoCss = String.raw`.spinner {
  position: relative;
  width: 60px;
  height: 60px;
  display: flex;
...
const dominoPreviewCss = dominoCss
  .replace('margin-left: -75px;', 'margin-left: 0; transform: translateX(-35px);')
  .replace('left: 60px;', '--left: 60px;')
  .replace('left: 50px;', '--left: 50px;')
  .replace('left: 40px;', '--left: 40px;')
  .replace('left: 30px;', '--left: 30px;')
  .replace('left: 20px;', '--left: 20px;')
  .replace('left: 10px;', '--left: 10px;')

export const seedDesigns: CustomDesign[] = [
  {
    id: 'custom-focus-spectrum-button',
    name: 'Focus Spectrum Button',
    category: 'Buttons',
    description: 'Layered dark button with animated letters, focus burst and spectral highlight.',
    html: focusButtonHtml,
    css: focusButtonCss,
    previewCss: focusButtonPreviewCss,
    tags: ['button', 'focus', 'glow', 'letters', 'animated'],
    source: 'seed',
  },
  {
    id: 'custom-domino-spinner',
    name: 'Domino Spinner',
    category: 'Loaders',
    description: 'Eight bars falling in sequence like domino tiles.',
    html: dominoHtml,
    css: dominoCss,
    previewCss: dominoPreviewCss,
    tags: ['loader', 'spinner', 'domino', 'animated'],
    source: 'seed',
  },
]
